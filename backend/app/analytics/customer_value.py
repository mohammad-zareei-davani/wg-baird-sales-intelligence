...
Ranks customers by Value Added (VA Amount) rather than raw Sell Price,
because VA is what the business actually keeps after paper/press/purchases
costs — it is the better proxy for "how valuable is this relationship".
"""
...
def customer_value_summary(df: pd.DataFrame, top_n: int = 15) -> dict:
    ...
    top_work_type = (
        df.groupby(["customer_id", "work_type"], as_index=False)["va_amount"]
        .sum()
        .sort_values("va_amount", ascending=False)
